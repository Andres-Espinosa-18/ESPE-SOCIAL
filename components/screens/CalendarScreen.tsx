...
import { Calendar as CalendarIcon, Info, ChevronLeft, ChevronRight, Clock, MapPin, X, Plus } from 'lucide-react';
...
export const CalendarScreen: React.FC = () => {
  ...
  // LOGIC FOR SIDE LIST: All future events sorted chronologically
  const getUpcomingEvents = () => {
      ...
      return events
        .filter(e => {
            // "2025-01-31" replaced to work across browsers slightly better if needed, but ISO is fine
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 h-full relative">
       ...
       {/* Events Side List (Chronological) */}
          ...
          <div className="space-y-3 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
             {upcomingEvents.length > 0 ? upcomingEvents.map((evt, i) => (
                <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:border-espe-green transition-colors group relative overflow-hidden">
                 ...
                 <p className="text-gray-400 text-center text-sm py-4">No tienes eventos próximos.</p>
             )}
          </div>
       </div>
